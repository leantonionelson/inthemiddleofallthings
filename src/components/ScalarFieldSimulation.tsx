...
type Source = {
  x: number;
  y: number;
  strength: number; // positive = hill, negative = well
};
...
const ScalarFieldSimulation: React.FC = () => {
  ...
  // Compute scalar field value at a point
  const computeScalarField = useCallback((x: number, y: number, sources: Source[]) => {
    let phi = 0;
    ...
    for (const s of sources) {
      ...
      phi += s.strength * invDist;
    }

    return phi;
  }, []);

  // Compute gradient of scalar field at a point
  const computeGradient = useCallback((x: number, y: number, sources: Source[]) => {
    ...
    for (const s of sources) {
      ...
      // gradient of phi_i = -s_i * (x-x_i, y-y_i) / (r^2 + eps)^(3/2)
      gx += factor * dx;
      gy += factor * dy;
    }
  ...
  }, [fieldStrength, damping, computeGradient]);
  ...
  const draw = useCallback(() => {
    ...
    // Compute field values for visualization
    ...
    for (let i = 0; i < SROWS; i++) {
      ...
      for (let j = 0; j < SCOLS; j++) {
        const x = (j + 0.5) * dx;
        const y = (i + 0.5) * dy;
        const phi = computeScalarField(x, y, sources);
        fieldValues[i][j] = phi;
    ...
    const range = Math.max(maxPhi - minPhi, 1e-6);
    ...
    for (let i = 0; i < SROWS; i++) {
      for (let j = 0; j < SCOLS; j++) {
        const x = j * dx;
        const y = i * dy;
        const phi = fieldValues[i][j];
        
        // Normalize to [0, 1]
        const normalized = (phi - minPhi) / range;
        
        // Map to color: positive = warm (hills), negative = cool (wells)
    ...
    ctx.fill();
  }, [getCanvasContext, computeScalarField]);
...
export default ScalarFieldSimulation;
